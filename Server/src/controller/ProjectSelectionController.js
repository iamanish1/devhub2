...
export const createProjectSelection = async (req, res) => {
  try {
    ...
    if (project.user.toString() !== req.user._id.toString()) {
      return res.status(403).json({ message: 'You can only configure selection for your own projects' });
    }

    // Check if selection already exists
    const existingSelection = await ProjectSelection.findOne({ projectId });
    if (existingSelection) {
      return res.status(400).json({ message: 'Selection configuration already exists for this project' });
    }

    // Validate required contributors
    if (!requiredContributors || requiredContributors < 1) {
      return res.status(400).json({ message: 'Required contributors must be at least 1' });
    }

    if (requiredContributors > 50) {
      return res.status(400).json({ message: 'Required contributors cannot exceed 50' });
    }

    // Validate criteria weights sum to 100
    const totalWeight = criteriaWeights.skillMatch + criteriaWeights.bidAmount + 
                       criteriaWeights.experience + criteriaWeights.availability;
    ...
    logger.info(`[ProjectSelection] Created selection config for project: ${projectId}`);
    ...
    logger.error(`[ProjectSelection] Error in createProjectSelection: ${error.message}`, error);
...
export const getProjectSelection = async (req, res) => {
  try {
    ...
    const isProjectOwner = selection.projectOwner._id.toString() === req.user._id.toString();
    const isSelectedUser = selection.selectedUsers.some(user => 
    ...
    logger.error(`[ProjectSelection] Error in getProjectSelection: ${error.message}`, error);
    res.status(500).json({ message: 'Internal server error', error: error.message });
  }
};
...
export const executeAutomaticSelection = async (req, res) => {
  try {
    ...
    if (project.user.toString() !== req.user._id.toString()) {
      return res.status(403).json({ message: 'You can only execute selection for your own projects' });
    }
    ...
    if (selection.status === 'in_progress' && !force) {
      return res.status(400).json({ message: 'Selection is already in progress' });
    }

    if (selection.status === 'completed' && !force) {
      return res.status(400).json({ message: 'Selection is already completed' });
    }
    ...
    logger.info(`[ProjectSelection] Starting automatic selection for project: ${projectId}`);
    ...
    const result = await selectionAlgorithm.executeAutomaticSelection(projectId, selectionConfig);
    ...
    logger.error(`[ProjectSelection] Error in executeAutomaticSelection: ${error.message}`, error);
    res.status(500).json({ message: 'Internal server error', error: error.message });
  }
};
...
export const manualSelection = async (req, res) => {
  try {
    ...
    logger.error(`[ProjectSelection] Error in manualSelection: ${error.message}`, error);
    res.status(500).json({ message: 'Internal server error', error: error.message });
  }
};
...
export const getRankedBidders = async (req, res) => {
  try {
    ...
    if (project.user.toString() !== req.user._id.toString()) {
      return res.status(403).json({ message: 'You can only view bidders for your own projects' });
    }
    ...
    const rankedBidders = await selectionAlgorithm.selectBidders(projectId, selectionConfig);
    ...
    logger.error(`[ProjectSelection] Error in getRankedBidders: ${error.message}`, error);
    res.status(500).json({ message: 'Internal server error', error: error.message });
  }
};
...
export const updateSelectionConfig = async (req, res) => {
  try {
    ...
    if (project.user.toString() !== req.user._id.toString()) {
      return res.status(403).json({ message: 'You can only update selection for your own projects' });
    }
    ...
    if (selection.status === 'completed') {
      return res.status(400).json({ message: 'Cannot update selection configuration after completion' });
    }

    // Update fields if provided
    if (selectionMode) selection.selectionMode = selectionMode;
    if (requiredContributors) selection.requiredContributors = requiredContributors;
    ...
    logger.info(`[ProjectSelection] Updated selection config for project: ${projectId}`);
    ...
    logger.error(`[ProjectSelection] Error in updateSelectionConfig: ${error.message}`, error);
    res.status(500).json({ message: 'Internal server error', error: error.message });
  }
};
...
export const getProjectOwnerSelections = async (req, res) => {
  try {
    ...
    console.error('❌ [ProjectSelection] Error in getProjectOwnerSelections:', error);
    res.status(500).json({ message: 'Internal server error', error: error.message });
  }
};
...
export const cancelSelection = async (req, res) => {
  try {
    ...
    if (project.user.toString() !== req.user._id.toString()) {
      return res.status(403).json({ message: 'You can only cancel selection for your own projects' });
    }
    ...
    if (selection.status === 'completed') {
      return res.status(400).json({ message: 'Cannot cancel completed selection' });
    }
    ...
    logger.info(`[ProjectSelection] Cancelled selection for project: ${projectId}`);
    ...
    logger.error(`[ProjectSelection] Error in cancelSelection: ${error.message}`, error);
    res.status(500).json({ message: 'Internal server error', error: error.message });
  }
};
